...
void setup()
{
  ...
  float prob;
  for (int i=0; i<num; i++) {
    pt[index++] = random(PI*3); // Random X axis rotation
    pt[index++] = random(PI*3); // Random Y axis rotation
 
    pt[index++] = random(40,80); // Short to quarter-circle arcs
    if(random(100)>90) pt[index]=(int)random(8,27)*10;
 
    pt[index++] = int(random(2,50)*4); // Radius. Space them out nicely
